...
if (onlyStack === 'network') {
  new NetworkStack(app, 'PrdGaibuNetworkStack', {
    env,
  });
} else if (onlyStack === 'security') {
  const networkStack = new NetworkStack(app, 'PrdGaibuNetworkStack', {
    env,
  });
  const securityStack = new SecurityStack(app, 'PrdGaibuSecurityStack', {
    env,
  ...
  const monitoringStack = new MonitoringStack(app, 'PrdGaibuMonitoringStack', { env });
  ...
  securityStack.addDependency(networkStack);
  const databaseStack = new DatabaseStack(app, 'PrdGaibuDatabaseStack', {
    env,
    vpc: networkStack.vpc,
  ...
  databaseStack.addDependency(securityStack);
  const monitoringStack = new MonitoringStack(app, 'PrdGaibuMonitoringStack', { env });
  const computeStack = new ComputeStack(app, 'PrdGaibuComputeStack', {
    env,
    vpc: networkStack.vpc,
  ...
  const monitoringStack = new MonitoringStack(app, 'PrdGaibuMonitoringStack', {
    env,
  });
